...
%% 4. Guitar Extraction (Focus on Guitar Range: 80 Hz - 1 kHz)
guitar_cutoff = [80, 1000]; % Hz
normalized_guitar = guitar_cutoff / (fs / 2);
[b_guitar, a_guitar] = butter(4, normalized_guitar, 'bandpass'); % 4th-order bandpass filter

% Extract guitar frequencies from the instrumental signal
guitar_only = filter(b_guitar, a_guitar, instrumental);

% Normalize the guitar signal to avoid clipping
guitar_only = guitar_only / max(abs(guitar_only));

%% 5. FIR and IIR Filter Combination
...
subplot(3,1,3);
spectrogram(guitar_only, 1024, [], [], fs, 'yaxis');
title('Spectrogram of Guitar Frequencies');
colorbar;

%% Save Outputs
audiowrite('instrumental.wav', instrumental, fs);
audiowrite('vocal_fir_iir.wav', fir_iir_vocal, fs);
audiowrite('guitar_only.wav', guitar_only, fs);
...
pause(5); % Wait for vocal playback to finish

disp('Playing the guitar-only track...');
sound(guitar_only, fs);

disp('Separation completed and saved as instrumental.wav, vocal_fir_iir.wav, and guitar_only.wav.');
